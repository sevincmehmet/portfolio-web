...
import { heroContent } from "../data/heroData"; // Yukarıda oluşturduğumuz veri dosyasını import et (yolunu kendine göre ayarla)

// Veri tipini belirliyoruz (TypeScript kullanıyorsan çok işine yarar)
interface HeroProps {
...
export default function Hero({ data = heroContent["mas"] }: HeroProps) {
  ...
  return (
    <section className="relative min-h-screen bg-[#0A0A0F] flex flex-col items-center justify-center px-4 md:px-8 py-24 overflow-hidden selection:bg-amber-500/30 selection:text-white font-sans">
      {/* --- SABİT ARKA PLAN IŞIKLARI --- */}
      <motion.div
        animate={{ scale: [1, 1.2, 1], opacity: [0.3, 0.5, 0.3] }}
        transition={{ duration: 10, repeat: Infinity, ease: "easeInOut" }}
        className="absolute top-[-10%] left-[-5%] w-[40vw] h-[40vw] bg-cyan-600/10 rounded-full blur-[120px] pointer-events-none"
      />
      <motion.div
        animate={{ scale: [1, 1.5, 1], opacity: [0.2, 0.4, 0.2] }}
        transition={{ duration: 15, repeat: Infinity, ease: "easeInOut" }}
        className="absolute bottom-[-10%] right-[-5%] w-[50vw] h-[50vw] bg-amber-600/10 rounded-full blur-[150px] pointer-events-none"
      />
      ...
      <AnimatePresence mode="wait">
        ...
        {viewMode === "vizyon" && (
          <motion.div
            ...
            className="max-w-[80rem] w-full mx-auto flex flex-col md:flex-row items-center relative z-10 pt-16"
          >
            {/* SOL BÖLÜM */}
            <div className="w-full md:w-3/5 flex flex-col justify-center relative z-20">
              ...
              >
                ...
                <div className="flex flex-wrap gap-3 mb-10">
                  {data.vizyon.skills.map((skill, i) => (
                    <motion.div
                      ...
                      className="px-4 py-2 rounded-xl bg-white/[0.03] border border-white/[0.05] text-[#F8F9FA] text-xs font-medium tracking-wide cursor-default backdrop-blur-sm"
                    >
                      {skill}
                    </motion.div>
                  ))}
                ...
                <div className="flex items-center gap-6">
                  <a
                    href="#projeler"
                    className="px-8 py-4 rounded-full bg-[#F8F9FA] text-[#0A0A0F] text-sm font-semibold tracking-wide hover:bg-amber-500 hover:text-white transition-all duration-300 shadow-[0_0_20px_rgba(255,255,255,0.1)]"
                  >
                    {data.vizyon.ctaText}
                  </a>
                  <a
            ...
            <div className="w-full md:w-2/5 h-[600px] relative mt-16 md:mt-0 flex justify-center items-center pointer-events-none">
              <motion.div
                ...
                className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[90%] aspect-square bg-gradient-to-tr from-amber-500/10 to-cyan-500/10 rounded-full blur-[80px] z-0"
              />
              <motion.div
                animate={{ rotate: 360 }}
                transition={{ duration: 40, repeat: Infinity, ease: "linear" }}
                className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[110%] aspect-square border border-white/[0.03] rounded-full z-0"
              >
                <div className="absolute top-0 left-1/2 -translate-x-1/2 w-1.5 h-1.5 bg-amber-500/50 rounded-full shadow-[0_0_10px_rgba(245,158,11,0.8)] blur-[1px]"></div>
              </motion.div>
              <motion.div
                animate={{ rotate: -360 }}
                transition={{ duration: 50, repeat: Infinity, ease: "linear" }}
                className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[130%] aspect-square border border-white/[0.02] rounded-full z-0"
              >
                <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-2 h-2 bg-cyan-500/40 rounded-full shadow-[0_0_12px_rgba(6,182,212,0.8)] blur-[1px]"></div>
              </motion.div>
              ...
              <motion.div
                ...
                className="relative w-[320px] md:w-[450px] aspect-square z-10 flex items-center justify-center"
              >
                <img
        ...
        {viewMode === "bento" && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -20, filter: "blur(10px)" }}
            transition={{ duration: 0.5, ease: "easeInOut" }}
            className="max-w-[75rem] w-full mx-auto grid grid-cols-1 md:grid-cols-12 auto-rows-[minmax(220px,auto)] gap-6 relative z-10 pt-16"
          >
            {/* SOL BÖLÜM: Ana Mesaj */}
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.8, ease: "easeOut", delay: 0.2 }}
              className="col-span-1 md:col-span-8 bg-white/[0.02] backdrop-blur-3xl border border-white/[0.05] rounded-[2.5rem] p-10 md:p-14 flex flex-col justify-center relative overflow-hidden group hover:border-white/10 hover:bg-white/[0.04] transition-all duration-700 shadow-2xl"
            >
              <div
            ...
            <motion.div
              ...
              className="col-span-1 md:col-span-4 row-span-2 relative flex items-center justify-center rounded-[2.5rem] overflow-hidden group"
            >
              <motion.div
                animate={{ rotate: 360 }}
                transition={{ duration: 25, repeat: Infinity, ease: "linear" }}
                className="absolute inset-0 border-[2px] border-dashed border-white/10 rounded-full scale-[1.35] opacity-50 group-hover:border-amber-500/30 transition-colors duration-700"
              />
              <motion.div
                animate={{ rotate: -360 }}
                transition={{ duration: 35, repeat: Infinity, ease: "linear" }}
                className="absolute inset-0 border-[1px] border-solid border-cyan-500/20 rounded-full scale-[1.5] opacity-30"
              />
              <div className="relative w-full h-full bg-white/[0.02] backdrop-blur-2xl border border-white/[0.08] rounded-[2.5rem] overflow-hidden shadow-[0_0_40px_rgba(0,0,0,0.5)] z-10">
                <img
            ...
            <motion.div
              ...
              className="col-span-1 md:col-span-4 bg-white/[0.02] backdrop-blur-3xl border border-white/[0.05] rounded-[2.5rem] p-8 flex flex-col justify-center relative overflow-hidden group hover:bg-white/[0.04] transition-colors"
            >
              <div className="absolute right-[-10%] top-[-10%] w-32 h-32 bg-cyan-500/10 rounded-full blur-[40px] pointer-events-none"></div>
            ...
            {/* ALT ORTA: Kahve (DOKUNULMADI) */}
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.8, delay: 0.5, ease: "easeOut" }}
              className="col-span-1 md:col-span-4 bg-gradient-to-br from-white/[0.03] to-transparent backdrop-blur-3xl border border-white/[0.05] rounded-[2.5rem] p-8 flex flex-col justify-between relative overflow-hidden group hover:border-white/10 transition-colors"
            >
              <motion.div
                animate={{ y: [-5, 5, -5] }}
                transition={{
                ...
                className="w-12 h-12 rounded-full bg-white/[0.05] border border-white/10 flex items-center justify-center text-xl shadow-inner mb-6 relative z-10"
              >
                ☕
              </motion.div>
              <div className="relative z-10">
        ...
        {viewMode === "zen" && (
          <motion.div
            ...
            className="max-w-7xl w-full mx-auto flex flex-col items-center justify-center relative z-10 py-10"
          >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-24 items-center w-full">
              ...
              <div className="relative flex justify-center items-center perspective-[2000px]">
                <div className="absolute w-[130%] aspect-square border border-white/[0.03] rounded-full animate-[spin_40s_linear_infinite]"></div>
                <motion.div
                  ...
                  className="relative w-72 md:w-[26rem] aspect-[3/4] z-10 group"
                >
                  <div className="absolute inset-0 bg-white/[0.02] backdrop-blur-[4px] border border-white/10 rounded-[2.5rem] shadow-[0_0_60px_rgba(0,0,0,0.6)] z-20"></div>
                  <img
                    src={data.zen.image}
                    alt="Mehmet Ali"
            ...
            {/* arka yazı */}
            <div className="absolute bottom-[-5%] text-[14vw] font-black text-white/5 tracking-tighter select-none pointer-events-none uppercase">
              {data.zen.bgText}
            </div>
          </motion.div>
        )}
